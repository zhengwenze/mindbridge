...
- 支持事件驱动多智能体、LangGraph bounded loop 与 custom fallback；通过 `AGENT_FRAMEWORK` 或管理员“运行切换”页面选择主运行时。
...
说明：事件驱动运行时入口为 `app/agents/event_driven_runtime.py`，LangGraph 入口为 `app/agents/langgraph_runtime.py`，`app/agents/runtime.py` 是 custom fallback。Docker Compose、`.env.example` 与 Python 配置均默认选择 `langgraph`，也可通过 `AGENT_FRAMEWORK` 指定启动运行时。RAG 默认使用 Chroma 本地持久化向量库做语义召回，同时用 BM25 做关键词召回，再融合并本地 rerank；Chroma 或 Ollama embedding 服务异常且未强制要求向量时，会自动回退到本地 BM25 + `hybrid_score` reranker，避免演示环境中断。
...
管理员可访问 `http://localhost:3000/admin/runtime`，在“运行切换”页面选择当前主运行方式。页面支持三个规范值：

- `event_driven_multi_agent`：事件驱动多智能体。
- `langgraph`：LangGraph bounded loop，也是系统默认值。
- `custom`：基础自研运行时，可用于兼容或应急场景。
...
# 查询当前运行方式、默认值和可选项
curl -u admin:000000 \
  http://127.0.0.1:8000/api/admin/agent-runtime

# 切换当前主运行方式
curl -u admin:000000 \
  -X PATCH \
  -H 'Content-Type: application/json' \
  -d '{"framework":"event_driven_multi_agent"}' \
  http://127.0.0.1:8000/api/admin/agent-runtime
```

切换从下一轮新对话开始生效，不会改变或中断正在进行的 SSE 流式输出。管理员选择仅保存在当前后端进程内；服务重启后会恢复为 `AGENT_FRAMEWORK` 指定的启动值，未配置时使用默认的 `langgraph`。

本功能只切换当前主运行方式，不新增 `event_driven_multi_agent → langgraph → custom` 三级自动降级机制，也不保存切换历史或审计记录。
...
```bash
curl -u admin:000000 http://127.0.0.1:8000/api/admin/knowledge/status
curl -u admin:000000 -X POST http://127.0.0.1:8000/api/admin/knowledge/rebuild-vector
curl -u admin:000000 -X POST http://127.0.0.1:8000/api/admin/knowledge/backup
```
...
```bash
curl -u stu0:000000 http://127.0.0.1:8000/api/agent/status
```
...
```bash
curl -u admin:000000 http://127.0.0.1:8000/api/admin/reports
```
