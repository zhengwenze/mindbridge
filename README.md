...
Excel 台账：openpyxl
邮件预警：SMTP / smtplib
前端：Next.js / React / Ant Design，Docker 开发环境通过独立 frontend 容器热重载
认证：Basic Auth
工具协议：MCP
```
...
也可以使用一键脚本，它会在 `3000` 被占用时自动选择 `3001-3005` 中的可用端口：
...
| `app` | MindBridge API | `8080` | 后端 API、鉴权、Agent、MySQL/Redis 业务链路 |
| `frontend` | Next.js dev server | `3000` | 前端开发服务器，挂载 `./frontend` 并支持热重载 |
...
开发时修改 `frontend/` 下的页面、组件或样式文件，Next.js dev server 会自动热重载浏览器里的界面，不需要重新构建或重启 Docker。前端容器会把 `/api/*` 和 `/actuator/*` 请求代理到 `app:8080`，后端接口仍可通过 `http://localhost:8080` 单独访问。如果使用 `FRONTEND_PORT=3001`，浏览器访问地址相应改为 `http://localhost:3001`。
...
```bash
docker compose exec app env MINDBRIDGE_BASE_URL=http://127.0.0.1:8080 \
...
docker compose exec app env \
  MINDBRIDGE_BASE_URL=http://127.0.0.1:8080 \
