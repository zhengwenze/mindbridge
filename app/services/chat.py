...
class ChatService:
    ...
    async def stream_chat(self, user: UserAccount, request: ChatRequest):
        ...
        yield sse(
            "meta",
            ChatStreamEvent(
                type="meta", sessionId=outcome.session.public_id
        ...
        async for token in self.ai.stream(outcome.response_messages):
            ...
            yield sse(
                "token",
                ChatStreamEvent(
                    type="token", sessionId=outcome.session.public_id, content=token
                ).model_dump(),
            )
        if assistant:
            raw_content = "".join(assistant)
            normalized_content, cited_sources = normalize_citations(
                raw_content, source_catalog
            )
            self.agent_harness.save_assistant_message(
                user, outcome.session, normalized_content
            )
            if cited_sources:
                yield sse(
                    "sources",
        ...
        yield sse(
            "done",
            ChatStreamEvent(
                type="done", sessionId=outcome.session.public_id
            ).model_dump(),
        )
